 ...
 * @Last Modified time: 2015-01-24 19:20:16
 */
$(function() {
    // 加载用户的关注
    var json_focus = '';
    ...
    //多图显示效果 
    $.each($('.photo_box'), function(index0, val) {
        $.each($(this).find('li'), function(index, val) {
            $(this).off('click').on('click', function(event) {
                ...
                var chooseleft = 0;
                if (index > 3) {
                    index > 5 ? chooseleft = -118 : chooseleft = (index - 3) * (-59);
                    detail.find('.picchoose ').css('margin-left', chooseleft);
                }
                if (totallength > 7) {
                    detail.find('.curright').off('click').on('click', function() {
                        if (chooseleft == 0 || chooseleft == -59) chooseleft += -59;
                        $(this).prev('div').find('ul').css('margin-left', chooseleft);
                    });
                    detail.find('.curleft').off('click').on('click', function() {
                        if (chooseleft == -118 || chooseleft == -59) chooseleft += 59;
                        $(this).next('div').find('ul').css('margin-left', chooseleft);
                    });
                }
                //加载功能和弹出大图
                _this.append('<i class="loadingb"></i>');
