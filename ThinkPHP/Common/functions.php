...
function todate($value){
    if(strtotime($value) < mktime(0, 0, 0, date("m")  , date("d"), date("Y"))){
        if(date('Y',strtotime($value))<date('Y')){
            $contentinput[$key]['date'] = date("Y年m月d日",strtotime($value));
        }else{
            $contentinput[$key]['date'] = date("m月d日",strtotime($value));
        }
      
      }else{
        if($howlong = (time() - strtotime($value) < 3600)){
             $contentinput[$key]['date'] = $howlong.'分钟前';
        }else{
             $contentinput[$key]['date'] = '今天'.date("H:i",strtotime($value));

        }
      } 
       return $contentinput[$key]['date']; 
}
//提取文件中文件名
function filename($uid,$position) {
    $dir = ('D://WWW'.__ROOT__.'/Uploads/Public/'.$uid.'/content/'.$position);
   // $dir = substr($dir, 1,-1);
    $dir1 = array('.','..');
    $filename = scandir($dir);
    
    $filenames = array_diff($filename, $dir1);        
    return $filenames;               
}
